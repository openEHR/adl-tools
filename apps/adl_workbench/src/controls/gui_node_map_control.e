...
feature -- Commands
	...
	roll_up_to_specialisation_level
		...
		require
			archetype_selected: kr.has_valid_selected_archetype
		do
			...
			if a_ti /= Void then
				...
				if a_node /= Void then
					...
					if c_o /= Void then
						...
						elseif kr.has_valid_selected_archetype then
					...
					elseif attached {C_QUANTITY_ITEM} a_node as c_q_i then
						if attached {EV_TREE_NODE} a_ti.parent as parent_ti then
							if attached {C_DV_QUANTITY} parent_ti.data as c_q2 then
								create pixmap_ext.make(0)
								if in_differential_mode and kr.has_valid_selected_archetype then
	...
	c_complex_object_string(a_node: C_COMPLEX_OBJECT): STRING
		...
		do
			...
			if a_node.is_addressable then
				...
				if kr.has_valid_selected_archetype and ontology.has_term_code (a_node.node_id) then
...
invariant
	...
	ontology_has_language: kr.has_valid_selected_archetype implies ontology.has_language (current_language)
