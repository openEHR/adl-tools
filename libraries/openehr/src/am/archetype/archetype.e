...
feature {AOM_POST_COMPILE_PROCESSOR, AOM_POST_PARSE_PROCESSOR, ARCHETYPE_VALIDATOR, ARCHETYPE_FLATTENER, C_XREF_BUILDER, EXPR_XREF_BUILDER, ARCH_CAT_ARCHETYPE} -- Validation
	...
	data_codes_index: HASH_TABLE [ARRAYED_LIST [C_PRIMITIVE_OBJECT], STRING]
			...
			def_it.do_all (
				agent (a_c_node: ARCHETYPE_CONSTRAINT; depth: INTEGER; idx: HASH_TABLE [ARRAYED_LIST [C_OBJECT], STRING])
					local
						key: STRING
					do
						if attached {C_PRIMITIVE_OBJECT} a_c_node as cpo and then attached {C_TERMINOLOGY_CODE} cpo.item as ccp
							and then (not ccp.any_allowed and ccp.code_count > 0)
						then
							across ccp.code_list as codes_csr loop
								...
								if not idx.has (key) then
									idx.put (create {ARRAYED_LIST[C_PRIMITIVE_OBJECT]}.make(0), key)
								end
								idx.item (key).extend (cpo)
							end
						end
					end (?, ?, Result),
				Void)
