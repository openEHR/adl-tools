...
feature -- Initialization
	...
	make (a_value: REAL_64; a_symbol: DV_CODED_TEXT)
...
feature -- Access
	...
	limits: REFERENCE_RANGE [DV_SCALE]
			-- limits of the scale enumeration, to allow comparison of a scale value to its limits.
		do
		end
...
feature -- Comparison
	...
	is_strictly_comparable_to (other: DV_SCALE): BOOLEAN
			-- two scales can be compared if they come from the same series
		do
			Result := symbol.is_comparable(other.symbol)
		end

	infix "<" (other: DV_SCALE): BOOLEAN
...
--| License.
--|
--| The Original Code is dv_scale.e.
--|
