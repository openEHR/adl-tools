	...
	create_bmm_generic_parameter_definition (a_bmm_schema: BMM_SCHEMA)
		do
			if attached conforms_to_type then
				create bmm_generic_parameter_definition.make_constrained (name, a_bmm_schema.class_definition (conforms_to_type))
			else
				create bmm_generic_parameter_definition.make (name, a_bmm_schema.class_definition (Any_type))
			end
		end
