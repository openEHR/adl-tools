...
feature -- Artefacts
	...
	all_paths (differential_view, editing_enabled: BOOLEAN): ARRAYED_LIST[STRING]
			-- path map down to primitive leaf nodes
		do
			Result := select_archetype (differential_view, editing_enabled).all_paths
		end

	data_value_paths (differential_view, editing_enabled: BOOLEAN): ARRAYED_LIST[STRING]
			-- paths down to DATA_VALUE level of tree
		local
...
feature {NONE} -- Flattening
	...
	expand_c_proxy_objects (flat_arch: like flat_archetype)
			-- expand all proxy objects in the flat archetype
		local
			c_obj: C_OBJECT
			cco_proxy_in_flat: C_COMPLEX_OBJECT_PROXY
		do
			across flat_arch.use_node_index as flat_use_node_path_csr loop
				across flat_use_node_path_csr.item as flat_use_nodes_for_path_csr loop
					cco_proxy_in_flat := flat_use_nodes_for_path_csr.item
					c_obj := flat_arch.object_at_path (flat_use_node_path_csr.key).safe_deep_twin
