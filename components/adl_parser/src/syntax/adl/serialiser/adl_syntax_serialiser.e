indexing
	...
	file:	"$URL: http://www.openehr.org/svn/ref_impl_eiffel/TAGS/Aug2007/components/adl_parser/src/syntax/adl/serialiser/adl_syntax_serialiser.e $"
...
feature -- Serialisation
	...
	serialise_initialise is
		...
		do
			...
			s.replace_substring_all("$title", "Archetype " + archetype.archetype_id.as_string)
			s.append(format_item(FMT_END_TITLE))

			-- meta-data
			s := format_item(FMT_META_ITEM).twin
			s.replace_substring_all("$name", "description")
			s.replace_substring_all("$content", "Generated by Ocean ADL validator parser - see www.OceanInformatics.biz")
	...
	serialise_archetype_id is
		...
		do
			...
			if archetype.has_adl_version or archetype.is_controlled or archetype.uid /= Void then
				arch_kw_str.append(" (")
				...
				-- uid
				if archetype.uid /= Void then
					if not arch_kw_str.is_empty then
						arch_kw_str.append ("; ")
					end
					arch_kw_str.append (symbol (SYM_UID)  + "=" + archetype.uid.out)
				end
