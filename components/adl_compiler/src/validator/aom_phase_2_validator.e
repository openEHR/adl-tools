	...
	io.put_string (">>>>> validate: C_OBJECT in child at " + terminology.annotated_path (co_child_diff.path, target_descriptor.archetype_view_language, True))
				...
				if attached {CONSTRAINT_REF} co_in_flat_anc as ccr and then not attached {CONSTRAINT_REF} co_child_diff as ccr2 then
					if attached {C_TERMINOLOGY_CODE} co_child_diff as ccp then
						add_warning (ec_WCRC, <<terminology.annotated_path (co_child_diff.path, target_descriptor.archetype_view_language, True)>>)
					else
						add_error (ec_VSCNR, <<co_in_flat_anc.generating_type, terminology.annotated_path (co_in_flat_anc.path, target_descriptor.archetype_view_language, True),
							co_child_diff.generating_type, terminology.annotated_path (co_child_diff.path, target_descriptor.archetype_view_language, True)>>)
					end
				...
				elseif attached {CONSTRAINT_REF} co_child_diff then
					...
					if not attached {C_TERMINOLOGY_CODE} co_in_flat_anc then
						...
						if not cref_conformance_ok then
							add_error (ec_VCRR, <<terminology.annotated_path (co_child_diff.path, target_descriptor.archetype_view_language, True),
